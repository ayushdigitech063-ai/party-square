...
export default function IndependenceDayProductDetail({ params }: PageProps) {
 ...
 const handleAddToCart = () => {
  addToCart({ ...product, id: String(product.id), numericPrice: unitPrice }, quantity);
  setCartMessage(true);
        ...
        <div className="bg-white rounded-3xl overflow-hidden shadow-lg border border-amber-200/60 grid grid-cols-1 md:grid-cols-2 gap-8 p-6 md:p-10">
          ...
          <div className="flex flex-col justify-between space-y-6">
            <div className="space-y-4">
              ...
              <p className="text-neutral-600 text-sm leading-relaxed font-light">{product.fullDesc}</p>
            </div>
            ...
            <div className="pt-4 border-t border-neutral-100 flex items-center justify-between">
              <div>
                ...
                <div className="flex items-center space-x-3 bg-neutral-100 border border-neutral-200 rounded-full px-3 py-1.5">
                  <button onClick={handleDecrement} className="w-6 h-6 rounded-full bg-white text-neutral-700 flex items-center justify-center hover:bg-amber-600 hover:text-white transition shadow-sm">
                    <Minus size={12} />
                  </button>
                  <span className="text-sm font-bold text-neutral-900 w-6 text-center">{quantity}</span>
                  <button onClick={handleIncrement} className="w-6 h-6 rounded-full bg-white text-neutral-700 flex items-center justify-center hover:bg-amber-600 hover:text-white transition shadow-sm">
                    <Plus size={12} />
                  </button>
                </div>
              </div>
              ...
              <div>
                <span className="text-[10px] uppercase text-neutral-400 block font-semibold tracking-wider">Total Price</span>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl border border-amber-100 animate-in fade-in zoom-in duration-200 relative">
            <button onClick={() => setIsModalOpen(false)} className="absolute top-5 right-5 text-neutral-400 hover:text-neutral-900 transition">
              <X size={20} />
            </button>
            ...
            {isConfirmed ? (
              ...
              <div className="space-y-6">
                <div className="space-y-2">
                  <span className="text-[10px] uppercase tracking-widest bg-amber-100 text-amber-800 px-3 py-1 rounded-full font-bold">Order Summary</span>
                ...
                <div className="flex space-x-3">
                  <button onClick={() => setIsModalOpen(false)} className="flex-1 bg-neutral-100 hover:bg-neutral-200 text-neutral-800 py-3 rounded-full font-bold text-xs uppercase tracking-wider transition">
                    Cancel
                  </button>
                  <button onClick={handleConfirmBooking} className="flex-1 bg-amber-600 hover:bg-amber-500 text-white py-3 rounded-full font-bold text-xs uppercase tracking-wider transition shadow-md">
                    Confirm Booking
                  </button>
                </div>
              </div>
