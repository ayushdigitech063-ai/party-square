...
export default function AllIndependenceDayProductsPage() {
  ...
  return (
    <div className="min-h-screen text-neutral-900 font-sans bg-[#FBF9F4] selection:bg-orange-600 selection:text-white pb-20">
      
      {/* Header / Back Navigation */}
      <div className="max-w-7xl mx-auto px-6 pt-8 pb-4 flex items-center justify-between">
        <Link 
          href="/Festivals/independenceday" 
          className="inline-flex items-center space-x-2 text-xs font-bold uppercase tracking-wider bg-white border border-orange-200 px-4 py-2 rounded-full hover:bg-neutral-950 hover:text-white transition shadow-sm"
        >
          <ArrowLeft size={14} />
          <span>Back to Independence Day</span>
        </Link>
        <span className="text-xs uppercase tracking-[0.2em] text-orange-800 font-bold">Patriotic Collection</span>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {independencedayProducts.map((product) => (
          <div key={product.id} className="bg-white border border-orange-200 rounded-3xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 flex flex-col justify-between group">
            <div className="relative h-64 w-full overflow-hidden bg-neutral-100 p-3">
              ...
              <span className="absolute top-5 left-5 bg-orange-600 text-white text-[10px] font-bold uppercase tracking-wider px-3 py-1 rounded-full shadow">
                Patriotic Special
              </span>
              <button className="absolute top-5 right-5 w-9 h-9 rounded-full bg-white/90 backdrop-blur-md flex items-center justify-center text-neutral-700 hover:text-orange-600 shadow transition">
                <Heart size={18} />
              </button>
            </div>
