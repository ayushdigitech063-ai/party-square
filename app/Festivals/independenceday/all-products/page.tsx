...
export default function AllIndependenceDayProductsPage() {
            ...
            <div key={product.id} className="bg-white border border-amber-200 rounded-3xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 flex flex-col justify-between group">
              <div className="relative h-64 w-full overflow-hidden bg-neutral-100 p-3">
                ...
                <button
                  ...
                  className="absolute top-5 right-5 w-9 h-9 rounded-full bg-white/90 backdrop-blur-md flex items-center justify-center shadow transition hover:scale-110 cursor-pointer"
                >
                  <Heart size={18} className={isLiked ? "fill-rose-500 text-rose-500" : "text-neutral-700"} />
                </button>
              </div>
  ...
  type="button"
  onClick={() =>
    addToCart({ ...product, id: String(product.id), numericPrice: parsePrice(product.price) }, 1)
  }
  title="Add to Cart"
