      ...
      <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {janmashtamiProducts.map((product) => (
          <div key={product.id} className="bg-white border border-amber-200/80 rounded-3xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 flex flex-col justify-between group">
            ...
            <Link href={`/Festivals/janmasthmi/${product.id}`} className="relative h-64 w-full overflow-hidden bg-neutral-100 p-3 block">
              ...
              <span className="absolute top-5 left-5 bg-amber-800 text-amber-100 text-[10px] font-bold uppercase tracking-wider px-3 py-1 rounded-full shadow">
                Janmashtami Special
              </span>
              <span className="absolute top-5 right-5 w-9 h-9 rounded-full bg-white/90 backdrop-blur-md flex items-center justify-center text-neutral-700 hover:text-amber-700 shadow transition">
                <Heart size={18} />
              </span>
            </Link>
