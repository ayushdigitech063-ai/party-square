...
import { Sparkles, CheckCircle, Star, Heart, ArrowRight, ShieldCheck, Gift, ChevronDown } from "lucide-react";
    ...
    <div className="min-h-screen bg-[#FAF8F5] text-neutral-900 font-sans">
        ...
        <div className="relative z-20 max-w-7xl mx-auto px-6 w-full">
          <div className="max-w-3xl space-y-8">
            ...
            <h1 className="font-serif text-5xl sm:text-6xl lg:text-7xl font-bold leading-tight drop-shadow-xl">
              Make Every Birthday <span className="text-[#DFBC71] italic">Unforgettable</span>
            </h1>

            <p className="text-neutral-200 text-lg max-w-2xl font-light leading-relaxed">
              Transform your special day into a magical wonderland. From whimsical kids' themes to luxurious milestone birthday setups, we create joy filled with vibrant colors and elegance.
            </p>
      ...
      <section id="packages" className="py-24 px-6 max-w-7xl mx-auto">
        <div className="text-center max-w-2xl mx-auto mb-20 space-y-4">
          <span className="text-xs uppercase tracking-[0.25em] text-[#C5A059] font-bold">Party Packages</span>
          <h2 className="font-serif text-5xl md:text-6xl font-bold">Choose Your Birthday Theme</h2>
          <p className="text-neutral-600 text-lg font-light">Explore our specialized decoration packages crafted to bring smiles and unforgettable memories.</p>
        </div>
          ...
          {birthdayPackages.map((pkg) => (
            <div
              ...
              className={`relative rounded-3xl bg-white border transition-all duration-300 p-8 flex flex-col justify-between shadow-xl hover:shadow-2xl ${
                pkg.popular ? "border-2 border-[#C5A059] ring-4 ring-[#C5A059]/10 scale-105 md:-translate-y-3" : "border-neutral-200"
              }`}
            >
              {pkg.popular && (
              ...
              <div>
                ...
                <h3 className="font-serif text-3xl font-semibold text-neutral-900 mb-3">{pkg.name}</h3>
                <p className="text-neutral-600 text-sm leading-relaxed mb-8">{pkg.desc}</p>
              </div>
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
            <div className="max-w-2xl">
              <span className="text-xs uppercase tracking-[0.25em] text-[#8C6D24] font-medium">Moments of Joy</span>
              <h2 className="font-serif text-5xl md:text-6xl text-neutral-900 mt-2 leading-tight">Kids & Special Birthday Gallery</h2>
              <p className="text-neutral-700 text-lg font-light mt-4">A glimpse into our joyous setups featuring wonderful decorations for kids and milestones.</p>
            </div>
            <Link href="/gallery" className="shrink-0 bg-white text-neutral-900 px-8 py-3.5 rounded-full font-bold text-xs uppercase tracking-widest border border-neutral-300 hover:border-[#C5A059] hover:text-[#8C6D24] transition flex items-center space-x-2 shadow-sm">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-5">
            {childGallery.map((img, idx) => (
              <div key={idx} className="aspect-square rounded-3xl overflow-hidden shadow-lg border border-white group">
                <img src={img} alt={`Child Birthday Scene ${idx + 1}`} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700 ease-out" />
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 md:px-16 text-center lg:text-left lg:flex lg:items-center lg:justify-between gap-12">
          ...
          <div className="space-y-6 lg:max-w-3xl">
            ...
            <p className="text-neutral-200 text-lg md:text-xl leading-relaxed max-w-2xl mx-auto lg:mx-0 font-light">
              Contact DreamDeco today to discuss your vision. Our expert stylists are excited to craft a bespoke, magical celebration that you and your guests will remember forever.
            </p>
          </div>
