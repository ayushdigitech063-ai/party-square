...
export default function YoungBirthdayPage({
  params,
}: PageProps) {
...
const [relatedStart, setRelatedStart] = useState(0);
  ...
  const isNewYearProduct = newYearCards.some(
    (item) => item.id == productId
  );

  const isHotelProduct = hotelCards.some(
    (item) => item.id == productId
  );

  const categoryName = isNewYearProduct
  ...
  const parsedOriginalPrice =
    originalPriceValue !== undefined
      ? Number(
          String(originalPriceValue).replace(
            /[^\d.]/g,
            ""
          )
        )
  ...
  const whatsIncluded: string[] =
    (product as any).whatsIncluded ?? [
      "Premium celebration decoration setup",
...
const relatedProducts = newYearCards.filter(
...
<div className="bg-white">
            ...
            <section className="mt-5 grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              <div className="bg-[#fff9e9] border border-[#eadba7] rounded-[13px] p-6 sm:p-8 flex flex-col justify-center min-h-[220px]">
                ...
                <p className="mt-3 text-[10px] leading-5 text-neutral-500 max-w-[450px]">
                  Create unforgettable celebrations with thoughtfully
                  designed decorations, premium setup and beautiful
          ...
          <section className="mt-5 grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div className="bg-[#fff9e9] border border-[#eadba7] rounded-[13px] p-8 flex flex-col justify-center">
              ...
              <p className="text-xs leading-5 text-neutral-500 mt-3">
                Celebrate your special moments with beautiful
                decorations and unforgettable memories.
              </p>
        ...
        {activeTab === "faq" && (
          <section className="mt-5 bg-white border border-[#e1ddd7] rounded-[13px] p-6">
            ...
            <p className="text-sm leading-6 text-neutral-600">
              Have questions about setup timing, venue requirements,
              themes, customization or decoration services? Contact
              our team before booking and we&apos;ll guide you
              through everything.
            </p>
...
<section className="mt-10 sm:mt-12 pb-6">
  ...
  {item.price}
              ...
              {/* BOOK */}
<Link
  href={`/services/birthday/young/${item.id}`}
